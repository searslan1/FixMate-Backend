...
class UserService {
    ...
    async updateProfile(userId, role, data) {
        ...
        if (role === 'MECHANIC') {
            if (data.workplace)
                updateData.workplace = data.workplace;
            if (data.services)
                updateData.services = data.services;
            if (data.brands)
                updateData.brands = data.brands;
            if (data.photos)
    ...
    async changePassword(userId, currentPassword, newPassword) {
        ...
        const isPasswordValid = await (0, password_1.comparePasswords)(currentPassword, user.password);
        if (!isPasswordValid) {
